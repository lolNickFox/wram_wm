...
wram_averages %>%
  filter(Trial != 1) %>%
  ggplot(aes(x = Trial, y = avr_WMC_error, group = Treatment, colour = Treatment)) + geom_point(size = 3) + geom_line(size = 1, alpha = 0.8) + facet_grid(. ~ Day) + theme_bw() + scale_x_continuous(breaks = seq(2, 4, 1), limits = c(1.5, 4.5)) + ylab("Working Memory Correct Errors")
...
wram_averages %>%
  filter(Trial != 1) %>%
  ggplot(aes(x = Trial, y = avr_RM_error, group = Treatment, colour = Treatment)) + geom_point(size = 3) + geom_line(size = 1, alpha = 0.8) + facet_grid(. ~ Day) + theme_bw() + scale_x_continuous(breaks = seq(2, 4, 1), limits = c(1.5, 4.5)) + ylab("Reference Memory Errors")
...
wram_averages_treatment %>%
  ggplot(aes(x = Day, y = treatment_WMC, group = Treatment, colour = Treatment)) + geom_point(size = 3) + geom_line(size = 1, alpha = 0.8) + geom_smooth(method = "lm", se = FALSE) + theme_bw() + scale_x_continuous(breaks = seq(2, 12, 1)) + ylab("Working Memory Correct Errors")
...
wram_averages_treatment %>%
  ggplot(aes(x = Day, y = treatment_RM, group = Treatment, colour = Treatment)) + geom_point(size = 3) + geom_line(size = 1, alpha = 0.8) + geom_smooth(method = "lm", se = FALSE) + theme_bw() + scale_x_continuous(breaks = seq(2, 12, 1)) + ylab("Reference Memory Errors")
